...
// The svg element appears before Mermaid finishes layout; screenshotting on
// existence alone captures a blank page. Wait for real height.
await page.waitForFunction(
  () => {
    const svg = document.querySelector(".mermaid svg");
    if (!svg) return false;
    const laidOut = svg.getBoundingClientRect().height > 200;
    const nodes = svg.querySelectorAll("g.node, .node").length;
    return laidOut && nodes > 10;
  },
  { timeout: 60_000 },
);
